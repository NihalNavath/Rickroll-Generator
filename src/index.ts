...
app.get("/data", async (req: Request<unknown, unknown, unknown, args>, res: Response) => {
    ...
    const result = await fetchFromDb(query.url);
...
export const updateCache = (url: string, params: any) => {
    info[url] = { title: params.title, description: params.description, type: params.type, expiry: params.expiry, ImgUrl: params.ImgUrl, createTime: params.cDateTime };
};
